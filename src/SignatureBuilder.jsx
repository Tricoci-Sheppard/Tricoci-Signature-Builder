...
const DEFAULTS = {
  ...
  address: "",
  logoUrl:
    "https://www.tricociuniversity.edu/wp-content/uploads/2025/10/TUBC-Sig-logo-black.png",
};
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          ...
          <div className="actions" style={{ display: "flex", gap: "0.5rem" }}>
            <button onClick={copySignature} className="btn" style={buttonPrimary}>
              Copy Signature
            </button>
            <button
              onClick={async () => {
          ...
          <div style={cardStyle}>
            ...
            <Row label="Campus address">
              <div>
                ...
                <div
                  style={{ fontSize: "0.8rem", color: "#6b7280", marginTop: "0.35rem" }}
                >
                  Please select the <strong>primary campus</strong> where you work.
                  This will automatically populate the mailing address in your
                  signature.
                </div>
              </div>
            </Row>
        ...
        >
          ...
          <div id="gmail-tab" style={{ display: "block" }}>
            ...
            <ol style={{ lineHeight: 1.6, paddingLeft: "1.2rem" }}>
              <li>
                Fill out the form and click <strong>“Copy Signature”</strong> (this copies a rich,
                clickable version).
              </li>
              <li>
                Open Gmail → ⚙️ <strong>Settings</strong> → <strong>See all settings</strong>.
              </li>
              <li>Scroll to <strong>Signature</strong> → <strong>Create New</strong>.</li>
              <li>
                Paste the signature into the box (⌘ + V on Mac / Ctrl + V on
                Windows).
              </li>
              <li>
          ...
          <div id="outlook-tab" style={{ display: "none" }}>
            ...
            <ol style={{ lineHeight: 1.6, paddingLeft: "1.2rem" }}>
              ...
              <li>
                Go to <strong>Settings → View all Outlook settings</strong> or <strong>File → Options →
                Mail → Signatures…</strong>.
              </li>
              <li>
                Paste the signature into the signature editor (⌘ + V on Mac /
                Ctrl + V on Windows).
              </li>
...
function buildHtml(values) {
  ...
  const phoneBlock = [
    values.mobile
      ? `<span style="font-weight:bold;color:${text}">Mobile:</span> <a href="tel:${safe(
          values.mobile
        )}" style="color:${text};text-decoration:none">${safe(values.mobile)}</a>`
      : "",
  ]
    .filter(Boolean)
    .join("<br/>");
...
<table cellpadding="0" cellspacing="0" role="presentation" style="font-family:Arial,Helvetica,sans-serif;color:${text};font-size:14px;line-height:1.35;width:100%;">
  <tr>
    <td valign="top" style="width:33%;padding-right:20px;text-align:left;">
      <img src="${safe(
        DEFAULTS.logoUrl
      )}" alt="Tricoci University" width="180" style="display:block;width:180px;height:auto;margin-bottom:8px;" />
      ${
        values.headshotUrl
          ? `<img src="${safe(
              values.headshotUrl
            )}" alt="Headshot" width="100" style="display:block;width:100px;height:auto;border-radius:8px;margin-top:8px;" />`
          : ""
      }
    </td>
    <td valign="top" style="width:67%;">
      <div style="font-size:24px;font-weight:800;color:${headline}">${safe(
        values.fullName
      )}</div>
      <div style="font-size:16px;font-weight:600;color:${text}">${safe(
        values.title
      )}</div>
      <div style="font-size:14px;line-height:20px;margin-top:8px">${formatAddress(
        safe(values.address)
      )}</div>
      ${phoneBlock ? `<div style="margin-top:8px">${phoneBlock}</div>` : ""}
      ${
        values.email
          ? `<div><span style="font-weight:bold;color:${text}">Email:</span> <a href="mailto:${safe(
              values.email
            )}" style="color:${text};text-decoration:none">${safe(values.email)}</a></div>`
          : ""
      }
      ${
        values.website
          ? `<div><a href="https://www.tricociuniversity.edu" style="color:${text};text-decoration:none">${safe(
              values.website
            )}</a></div>`
          : ""
      }
